...
for i in tqdm(range(num_wells)):
    ...
    #w = np.isnan(radial_intensity_raw[i]) 
    #radial_intensity_raw[i][np.isnan(radial_intensity_raw[i])]=0. # turn nans to 0
    radial_intensity_int.append(interpolate.interp1d(x0, y)) #interpolate
    radial_intensity.append(radial_intensity_int[i](xnew))

    #plt.plot(linspace(0,1,np.size(radial_intensity)),radial_intensity)
...
plt.colorbar()
color_plot_radial_profile.savefig('color_plot_radial_profile_Replicate2.pdf',bbox_inches = "tight")   # save as .eps
...
delta_intensity_vs_radius.savefig('delta_intensity_vs_radius_Replicate2.pdf',bbox_inches = "tight")   # save as .eps
plt.show() 
