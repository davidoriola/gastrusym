...
def phi2_fit_nosignal(phi20, T1):
    phi0=1-phi20
    return (1-phi0)*np.exp(-T1)
...
def combo_phi2_fit0(comboData, T24, T48):
    
    extract1 = comboData[:5] # first data
    extract2 = comboData[5:10] # second data
    extract3 = comboData[10:15] # second data
    
    result24 = phi2_fit_nosignal(extract1, T24)
    result48 = phi2_fit_nosignal(extract2, T48)
    result72 = phi2_fit_nosignal(extract3, T48+(T48-T24))
...
for train_index, test_index in kf.split(comboX):
    ...
    try:
        fittedParameters0,pcov = curve_fit(combo_phi2_fit0,x_train, y_train,bounds=((0.001,0.001),(inf,inf)))
...
timescale = 24/(fittedParameters[1]-fittedParameters[0]) # (time in hours equivalent to 1 u.a. of time)
q=1/timescale #(1/h)
error_q = (np.sqrt(pow(error[0],2)+pow(error[1],2)))/24 #(1/h)
time_array = [fittedParameters[0]*timescale,fittedParameters[1]*timescale,(fittedParameters[1]+(fittedParameters[1]-fittedParameters[0]))*timescale] # time array in signalling time
...
for i in range(0,np.size(phi0_array,0)):
    ...
    #params = alpha # Bundle parameter for ODE Solver
    sol.append(odeint(three_state_model, y0, t)) # solve

time_point_y = np.linspace(-0.05,1)
...
plt.plot(phi0_array,phi2_fit_nosignal(phi0_array,fittedParameters[0]),'-',color='gray')
plt.plot(phi0_array,phi2_fit_nosignal(phi0_array,fittedParameters[1]),'-',color='cadetblue')
plt.plot(phi0_array,phi2_fit_nosignal(phi0_array,fittedParameters[1]+(fittedParameters[1]-fittedParameters[0])),'-',color='black')
...
rc('axes',linewidth=2)  # box thickness
rc('font',size = 15)   # font size ticks
plt.axis([0,60,-0.04,1])    # range of the y and x axis ([xmin,xmax,ymin,ymax])
...
plt.plot(fittedParameters[0]*time_point_x*timescale ,time_point_y,'--',color='gray')    #no signal
plt.plot(fittedParameters[1]*time_point_x*timescale ,time_point_y,'--',color='cadetblue')   #no signal
