...
- Fusion of 24h and 48h gastruloids: The data and the analysis can be found in the folder `Analysis_Fusion_24h_48h_gast`
- Fusion of sorted T+ and T- aggregates: The data and the analysis can be found in the folder `Analysis_Fusion_Tp_Tm`. This folder contains the scripts to analyse the homotypic and heterotypic fusions using the morphometric data (`.json` and `.csv` output files) generated by [MOrgAna](https://github.com/LabTrivedi/MOrgAna). Additionally, the script `Analysis_rheological_parameters` combines the nanoindentation data with the fusion data to extract the surface tension $\gamma$ and shear viscosity $\eta$ of the aggregates. 
