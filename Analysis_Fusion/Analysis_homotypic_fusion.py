...
folder_dataTm = os.path.join(dir_path,'DataTmTm')
folder_dataTp = os.path.join(dir_path,'DataTpTp')

# gastruloids to analyze for TpTp and TmTm fusions

gastrsTm = ['g03A','g06A','g06D','g06E','g06F','g06G','g06H','g07C','g07E','g07F','g07G'] # negneg
gastrsTp = ['g06A','g06B','g06C','g08A','g08C','g08D','g08E']  # pospos

# ******************************************************************
# From now on in the code 24 -> TmTm and 48 -> TpTp
# ******************************************************************

num_gast24 = len(gastrsTm) # number of gastruloids 
num_gast48 = len(gastrsTp) # number of gastruloids 

# Get the filenames of all the .csv files with the morphometry
filenames24 = [os.path.join(folder_dataTm,g+'_morpho_straight_params.csv') for g in gastrsTm]
filenames48 = [os.path.join(folder_dataTp,g+'_morpho_straight_params.csv') for g in gastrsTp]
...
popt24,pcov24 = curve_fit(solNumSin2,time,sin2_mean24,bounds=((0.01,0.1), (np.inf,np.inf)))
popt48,pcov48 = curve_fit(solNumSin2,time,sin2_mean48,bounds=((0.01,0.1), (np.inf,np.inf)))
...
# Viscocapillary velocity  vc = eta/gamma
...
plt.plot(time,sin2_mean24,linewidth=2,label="-/- (n="+str(num_gast24)+")", color = 'gray')  # plot 
...
plt.plot(time,sin2_mean48,linewidth=2,label="+/+ (n="+str(num_gast48)+")", color = 'green')  # plot 
...
plt.show()   
fsin2.savefig('Fusion_curve_mean_TmTm_TpTp.pdf',bbox_inches = "tight")   # save as .eps

# Ouput parameters

output_sorting = pd.DataFrame({'Condition':['Tminus','Tplus'],'lEC (um)':[lEC24[0],lEC48[0]],'err_lEC (um)':[error_lEC24[0],error_lEC48[0]],'vc (um/min)':[vc24[0],vc48[0]],'err_vc (um/min)':[error_vc24[0],error_vc48[0]],'radius (um)':[radius_mean24[0],radius_mean48[0]], 'radius std (um)':[radius_std24[0],radius_std48[0]],'nsamples':[num_gast24,num_gast48],'tau (h)':[popt24[0],popt48[0]],'beta':[popt24[1],popt48[1]]})
output_sorting.to_csv('output_fits_Tplus_Tminus.csv')

# Output sin2 curves

df_sin2_Tminus = pd.DataFrame({'Time (h)':time,'Sin2theta_mean':sin2_mean24,'Sin2theta_std':sin2_std24})
df_sin2_Tplus = pd.DataFrame({'Time (h)':time,'Sin2theta_mean':sin2_mean48,'Sin2theta_std':sin2_std48})

df_sin2_Tminus.to_csv('output_sin2_Tminus.csv')
df_sin2_Tplus.to_csv('output_sin2_Tplus.csv')
